...
public class BaseDBDao {
    ...
    private final static boolean IS_AUTO_SET_TIME = true;
     ...
     */
    private <T> String getTableName(@Nonnull Class<T> clazz) {
        try {
    ...
    /**
     * 根据泛型实体的class获得该实体对应表中主键名称
     *
     * @param <T>   the type parameter
     * @param clazz the typ class
     * @return the primary key
     */
    private <T> EntitySetting getPrimaryKey(@Nonnull Class<T> clazz) {
        Stream<EntitySetting> entityInfo = getEntityInfo(clazz);
        Optional<EntitySetting> first = entityInfo.filter(EntitySetting::getWasPrimaryKey).findFirst();
     ...
     * @return the boolean
     */
    private boolean isAutoGenerated(Object entity) {
        Class<?> clazz = entity.getClass();
        Stream<EntitySetting> entitySetting = getEntityInfo(clazz);
        return entitySetting.filter(EntitySetting::getWasPrimaryKey).filter(EntitySetting::getAutoIncrease).count() > 0;
    }
     ...
     * @return
     */
    private <T> Stream<EntitySetting> getEntityInfo(Class<T> clazz) {
        Field[] fields = ReflectionUtils.getFields(clazz);
        Stream<EntitySetting> pairStream = Arrays.stream(fields)
                .map(field -> {
                    Column column = field.getAnnotation(Column.class);
                    if (ObjectUtils.isNull(column)) {
                        Method method = ReflectionUtils.getGetterMethod(clazz, field.getName());
                        if (method == null) {
                            return null;
                        } else {
                    ...
                    if (ObjectUtils.isNull(id)) {
                        ...
                        if (method == null) {
                            ...
                            generatedValue = method.getAnnotation(GeneratedValue.class);
                        }
                    } else {
                    ...
                    if (ObjectUtils.isNotNull(generatedValue)) {
                        ...
                        autoIncrease = ObjectUtils.isNull(strategy) || strategy.equals((GenerationType.AUTO));
                    }
                    boolean wasPrimaryKey = ObjectUtils.isNotNull(id);
                    return new EntitySetting(clazz, column, field, wasPrimaryKey, autoIncrease);
                })
    ...
    private class EntitySetting<T> {
        ...
        public EntitySetting(Class<T> clazz, Column column, Field field, Boolean wasPrimaryKey, Boolean autoIncrease) {
            ...
            this.column = column;
            this.field = field;
            this.wasPrimaryKey = wasPrimaryKey;
    ...
    public String getNewVersion(Object entity) throws UnsupportedEncodingException, NoSuchAlgorithmException {
        try {
            Stream<EntitySetting> entityInfo = getEntityInfo(entity.getClass());
            String valueStr = entityInfo.filter(setting ->
                    !StringUtils.equalsIgnoreCase(setting.getField().getName(), "version")
                            && !setting.getWasPrimaryKey())
                    .sorted(Comparator.comparing(setting -> setting.getField().getName()))
                    .map(setting -> {
                        Object value = ReflectionUtils.getFieldValue(entity, setting.getField().getName());
                        value = ObjectUtils.isNull(value) ? "" : value;
                        return value.toString();
                    }).collect(Collectors.joining());
            String version = MessageDigestUtils.md5(valueStr);
            return version;
        } catch (Exception ex) {
            throw ex;
        }
    ...
    public boolean save(@Nonnull Object entity) throws Exception {
        try {
            String primaryKey = this.getPrimaryKey(entity.getClass()).getField().getName();
    ...
    public boolean insertOne(Object entity, boolean createId) throws Exception {
        try {
            if (!isAutoGenerated(entity) && createId) {
                String primaryKey = this.getPrimaryKey(entity.getClass()).getField().getName();
    ...
    private boolean insert(@Nonnull Object entity) throws Exception {
        return this.insert(entity, IS_AUTO_SET_TIME);
    }

    /**
     * Insert boolean.
     * <p>
     * 修改的insert方法，如果为null，则不会添加字段，处理默认值不生效的问题
     *
     * @param entity the entity
     * @return the boolean
     * @throws Exception the exception
     */
    private boolean insert(@Nonnull Object entity, boolean isAutoSetTime) throws Exception {
        try {
            Class clazz = entity.getClass();
            ...
            if (ObjectUtils.isNotEmpty(table)) {
                ...
                List<String> parametersSql = new ArrayList<>();
                Map<String, Object> parameters = new HashMap<>();
                Stream<EntitySetting> entityInfo = getEntityInfo(clazz);
                entityInfo
                        .filter(setting -> {
                            String field = setting.getField().getName();
                            Object fieldValue = ReflectionUtils.getFieldValue(entity, field);
                            return ObjectUtils.isNotNull(fieldValue);
                        })
                        .forEach(setting -> {
                            String field = setting.getField().getName();
                            String column = setting.getColumn().name();
                            Object value = ReflectionUtils.getFieldValue(entity, field);
                            keySql.add(column);
                            parametersSql.add(":".concat(field));
                ...
                String keyStr;
                if (keySql.size() > 0) {
                    keyStr = keySql.stream().collect(Collectors.joining(","));
                } else {
                    throw new Exception(String.format("The class named %s does not contain any column.", clazz.getName()));
                }
                String parametersStr;
                if (parametersSql.size() > 0) {
                    parametersStr = parametersSql.stream().collect(Collectors.joining(","));
                } else {
                    throw new Exception(String.format("The class named %s does not contain any column.", clazz.getName()));
                }
                sql = sql.append("(").append(keyStr).append(") VALUES (").append(parametersStr).append(" )");

                if (isAutoSetTime) {
                    parameters.put("createTime", DateUtils.getNow());
                    parameters.put("updateTime", DateUtils.getNow());
                }
    ...
    public <T> boolean insertMany(@Nonnull List<T> tList, @Nonnull int flushSize) throws Exception {
        ...
        try {
            ...
            String primaryKey = this.getPrimaryKey(clazz).getField().getName();
            session = getSession();
    ...
    public boolean replace(@Nonnull Object entity, Boolean createId) throws Exception {
        ...
        Table table = (Table) clazz.getAnnotation(Table.class);
        if (ObjectUtils.isNotEmpty(table)) {
            EntitySetting setting = getPrimaryKey(clazz);
            String primaryKey = setting.getField().getName();
            Long primaryValue = DataSwitch.convertObjectToLong(ReflectionUtils.getFieldValue(entity, primaryKey), 0L);
            ...
            Arrays.stream(ReflectionUtils.getFields(clazz)).forEach(field -> {
                Column column = field.getAnnotation(Column.class);
                if (ObjectUtils.isNull(column)) {
            ...
            String insertKey = insert.stream().collect(Collectors.joining(","));
            String insertValue = insert.stream().map(str -> ":" + str).collect(Collectors.joining(","));
            String updateStr = update.stream().filter(field -> !StringUtils.equalsIgnoreCase(field, primaryKey)
                    || !StringUtils.equalsIgnoreCase(field, "createTime"))
            ...
            if (IS_AUTO_SET_TIME) {
                parameters.put("createTime", DateUtils.getNow());
                parameters.put("lastModifyTime", DateUtils.getNow());
                parameters.put("updateTime", DateUtils.getNow());
            }
    ...
    private int update(@Nonnull Object entity, boolean isFilterNull) throws Exception {
        ...
        String primaryKey = this.getPrimaryKey(clazz).getColumn().name();
        ...
        Stream<EntitySetting> entityInfo = getEntityInfo(clazz);
        entityInfo.filter(setting -> {
            if (IS_AUTO_SET_TIME) {
                String name = setting.getField().getName();
                if (StringUtils.equalsIgnoreCase(name, "updateTime")) {
                    ReflectionUtils.setFieldValue(entity, "updateTime", DateUtils.getNow());
                    return true;
            ...
            if (!parameters.containsKey(fieldName)) {
                ...
                if (!primaryKey.equalsIgnoreCase(column)) {
                    update.add(column.concat("=:").concat(fieldName));
                }
                if (field.getType().equals(String.class)) {
                    value = DataSwitch.convertObjectToString(value);
                }
                parameters.put(fieldName, value);
            }
        });
        sbSql.append(update.stream().collect(Collectors.joining(",")));
    ...
    private <T> String getFindByIdSql(@Nonnull Class<T> clazz) {
        try {
            ...
            final String primaryKey = this.getPrimaryKey(clazz).getColumn().name();
...
//region Find field
     ...
     * @return the string
     */
    private <T> String createFindFieldsSqlString(Class<T> clazz, Map<String, Object> parameters, String... fields) {
        try {
            String tableName = this.getTableName(clazz);
     ...
     * @return the string
     */
    private <T> String createFindSql(@Nonnull Class<T> clazz, Map<String, Object> parameters, Order... orders) {
        try {
            Stream<EntitySetting> entityInfo = getEntityInfo(clazz);
            String columns = entityInfo.map(EntitySetting::getColumn).map(Column::name).collect(Collectors.joining(","));
            String tableName = getTableName(clazz);
            StringBuilder sqlBuilder = new StringBuilder();
...
//    }
    ...
    public <T> List<Long> checkForeignKeysUsedState(Class<T> clazz, List<Long> foreignKeyValues, Class<?>... classes) throws Exception {
        try {
            ...
            List<String> filterTables = Arrays.stream(classes).filter(filterClass -> {
                if (filterClass == null) {
                    return false;
                }
                Entity entity = filterClass.getAnnotation(Entity.class);
                return entity != null;
            }).map(filterClass -> {
                Table table = filterClass.getAnnotation(Table.class);
                return table.name();
            }).distinct().collect(Collectors.toList());

            Map<String, Map<String, List<String>>> foreignKeySetting = SessionMap.getValue("foreignKeySetting");
            ...
            foreignKeyDetail.entrySet().stream()
                    ...
                    .forEach(entry -> {
                        String tableName = entry.getKey();
                        final List<String> foreignKeys = entry.getValue();
                        foreignKeys.stream().forEach(foreignKey -> {
                            StringBuilder innerSql = new StringBuilder();
                            innerSql.append("SELECT DISTINCT ").append(foreignKey)
                                    .append(" id,'" + tableName + "' type from ")
                                    .append(tableName).append(" WHERE ")
                                    .append(foreignKey).append(where);
                            selectSql.add(innerSql.toString());
                        });
                    });

//            tables.forEach((tableName, foreignKey) ->
//            {
//                if (!filterTables.contains(tableName)) {
//                    StringBuilder innerSql = new StringBuilder();
//                    innerSql.append("SELECT DISTINCT ").append(foreignKey).append(" num from ")
//                            .append(tableName).append(" WHERE ").append(foreignKey).append(where);
//                    selectSql.add(innerSql.toString());
//                }
//            });
